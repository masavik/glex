...
-spec be_ok({ok, DON} | {error, any()}) -> DON.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DOS}) -> DOS.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DOV)) -> DOV.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
